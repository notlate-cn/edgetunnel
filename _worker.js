import * as 个人输出 from './worker/personal-output.js';
			...
			if (env.KV && typeof env.KV.get === 'function') {
				...
				if (区分大小写访问路径 === 加密秘钥 && 加密秘钥 !== '勿动此默认密钥，有需求请自行通过添加变量KEY进行修改') {//快速订阅
					...
					if (用户客户端请求订阅 || 订阅转换后端请求订阅 || 作为优选订阅生成器) {
						config_JSON = await 读取config_JSON(env, host, userID, UA);
						const 个人订阅配置 = await 个人输出.读取个人订阅配置(env);
						...
						let 订阅内容 = '';
						const 个人输出依赖 = { 获取传输路径参数值, 获取传输协议配置, 生成随机IP, 整理成数组 };
						const 个人输出结果 = await 个人输出.准备个人输出({ request, url, env, config_JSON, 订阅TOKEN, 订阅类型, 个人订阅配置, deps: 个人输出依赖 });
						if (个人输出结果.内容) {
							订阅内容 = 个人输出结果.内容;
						} else
						if (订阅类型 === 'mixed') {
							...
							订阅内容 = 其他节点LINK + 完整优选IP.map(原始地址 => {
								...
								节点备注 = 个人输出.添加个人节点地址到备注(节点备注, 节点地址, 个人订阅配置);
						...
						if (订阅类型 === 'singbox') {
							...
							if (!个人输出结果.使用主配置) 订阅内容 = 个人输出.应用个人文本配置(订阅内容, 个人订阅配置);
